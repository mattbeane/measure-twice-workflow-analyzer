...
| Thing | Where it goes |
|-------|---------------|
| Your workflow data | Read from your local file or pasted into the browser. Sent only to the AI provider you select (Anthropic, OpenAI, or Google) for analysis. |
...
- Use an API key issued under your organization's commercial agreement with the provider (e.g. an Anthropic Workspace, an OpenAI organization with the no-training default, or a Google Workspace Vertex/AI Studio account). Most providers do not train on inputs under their commercial agreement; verify per provider before running on sensitive material.
...
1. **Provider API calls.**
   ...
   - **Browser tool** (`web/index.html`): to whichever of the three provider endpoints you selected (`api.anthropic.com`, `api.openai.com`, or `generativelanguage.googleapis.com`). The page never calls a provider you didn't pick.
